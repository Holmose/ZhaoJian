...
ZhaoJian 不是单纯的算命工具，也不是只服务情感领域的聊天分析器。
...
```bash
cd backend
python scripts/zhaojian_cli.py "我现在想做一个东方哲学增强型推演系统，未来这个项目能不能做大？" --domain strategy --goal "判断项目路线" --out reports/zhaojian
```

输出：

```text
reports/zhaojian/zhaojian_YYYYMMDD_HHMMSS.md
reports/zhaojian/zhaojian_YYYYMMDD_HHMMSS.json
```
...
```text
backend/app/zhaojian/
  __init__.py
  state_model.py              # ZhaoJianState 数据结构
...
当前版本为 **V0.4.0 / V1-V4 Prototype Completed**，已在本地 CLI 与 `/api/zhaojian/run` 跑通完整链路：
...
1. 做 V5 示例库与测试体系。
2. 替换前端遗留 MiroFish 品牌为 ZhaoJian。
